...
const Sidebar = () => {
  ...
  return <aside className="fixed left-0 top-0 h-screen w-64 glass-heavy flex flex-col z-30">
      {/* Logo */}
      ...
      <nav className="flex-1 px-4 py-5 overflow-y-auto space-y-6">
        {menuSections.map(section => <div key={section.label}>
            ...
            return <li key={item.label}>
                  ...
                  x: active ? 0 : 2
                }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 relative ${active ? "bg-primary/8 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-foreground/5"}`}>
                        {active && <motion.div layoutId="sidebar-active" className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-primary" transition={{
                    type: "spring",
                    stiffness: 300,
                    damping: 30
                  }} />}
      ...
      <div className="px-4 pb-4 space-y-0.5 border-t border-border/50 pt-4">
        ...
        <Link to="/settings">
          <motion.div whileHover={{
          x: isActive("/settings") ? 0 : 2
        }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 relative ${isActive("/settings") ? "bg-primary/8 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-foreground/5"}`}>
            {isActive("/settings") && <motion.div layoutId="sidebar-active" className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-primary" />}

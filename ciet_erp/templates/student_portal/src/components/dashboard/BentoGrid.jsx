...
const BentoGrid = () => {
  ...
  const cgpa = data.cgpa ? parseFloat(data.cgpa) : 0.0;
  const projectCount = data.projects ? data.projects.length : 0;
  const certCount = data.certifications ? data.certifications.length : 0;
  const topCerts = data.certifications ? data.certifications.slice(0, 2).map(c => c.title) : [];
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div>
            ...
            {cgpa > 0 && (
              <div className="flex items-center gap-1 mt-2 text-success">
                <TrendingUp size={14} />
                <span className="text-xs font-medium font-[Inter,sans-serif]">Excellent</span>
              </div>
            )}
          </div>
        </div>
      ...
      <motion.div custom={1} variants={cardVariants} initial="hidden" animate="visible" className="glass-card p-6">
        ...
        <div className="flex items-end gap-1 h-12 mb-3">
          ...
          height: `${Math.min(h, projectCount * 20 + 20)}%`
        }} transition={{
          delay: 0.5 + i * 0.05,
          duration: 0.4
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {topCerts.length > 0 ? (
            topCerts.map((cert, idx) => (
              <span key={idx} className="px-3 py-1.5 rounded-lg bg-foreground/5 text-xs font-medium text-foreground border border-border/50 font-[Inter,sans-serif] truncate max-w-full">
                {cert}
              </span>
            ))
          ) : (
            <span className="px-3 py-1.5 rounded-lg bg-foreground/5 text-xs font-medium text-muted-foreground border border-border/50 font-[Inter,sans-serif]">
              No Certifications
            </span>
          )}
        </div>
        <p className="text-sm text-muted-foreground font-[Inter,sans-serif]">
          <span className="text-foreground font-bold">{certCount}</span> Verified Credentials
        </p>
      </motion.div>
    </div>;
};
